...
describe("builder toolbars", () => {
    ...
    it("places a visible Quick Build stamp from the default build tool", async () => {
        ...
        expect(screen.getByTestId("quick-build-bake-batch")).toHaveTextContent("Optimize");
        expect(screen.getByTestId("quick-build-bake-batch")).toHaveAttribute(
            "aria-label",
            "Optimize Quick Build stamps for play",
        );
        expect(screen.getByTestId("quick-build-clear-bakes")).toHaveTextContent("Restore");
        expect(screen.queryByTestId("quick-build-bake-status")).not.toBeInTheDocument();
