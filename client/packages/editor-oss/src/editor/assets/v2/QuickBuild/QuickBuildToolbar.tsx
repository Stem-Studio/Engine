...
export const QuickBuildToolbar = ({
  ...
  const bakeQuickBuildBatch = useCallback(async () => {
    ...
    if (!batch) {
      showToast({
        type: "error",
        body: "Add Quick Build stamps before baking.",
      });
      refreshLiveBatch();
      refreshSceneSummary();
      return;
  ...
  const clearQuickBuildBakes = useCallback(async () => {
    ...
    try {
      ...
      showToast({
        type: "error",
        body: "Could not clear Quick Build baked batches.",
      });
      refreshLiveBatch();
      refreshSceneSummary();
    }
  ...
  const bakeDisabledReason =
    sceneSummary.stampCount === 0 ? "Add Quick Build stamps before baking." : "";
  const clearBakesDisabledReason =
    sceneSummary.bakedBatchCount === 0 ? "No baked Quick Build batches to clear." : "";
  const bakeTooltip =
    bakeDisabledReason ||
    `Bake ${sceneSummary.stampCount} Quick Build stamp${sceneSummary.stampCount === 1 ? "" : "s"}`;
  const clearBakesTooltip =
    clearBakesDisabledReason ||
    `Clear ${sceneSummary.bakedBatchCount} baked Quick Build batch${sceneSummary.bakedBatchCount === 1 ? "" : "es"}`;
  ...
  return (
    ...
    >
      ...
      <PanelDivider />
      <BakeGroup aria-label="Quick Build bake controls">
        <Tooltip text={bakeTooltip} height="auto">
          <UtilityButton
            type="button"
            aria-label="Bake Quick Build batch"
        ...
        <Tooltip text={clearBakesTooltip} height="auto">
          <UtilityButton
            type="button"
            aria-label="Clear Quick Build baked batches"
...
const BakeGroup = styled.div`
  display: grid;
  grid-template-columns: 34px 34px 28px;
  gap: 6px;
  align-items: center;
  flex: 0 0 auto;
`;
