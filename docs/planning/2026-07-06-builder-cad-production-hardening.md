...
### A1. Undo/redo desyncs BIM Plan visuals from data — [x] fix
...
### A2. Single source of truth for plan geometry on save/load — [x] decide + enforce
...
### A3. Scene-tree edits can orphan/resurrect plan nodes — [x] guard
...
### A4. Mode state machine for the four surfaces — [x] refactor
...
### A5. Typing and lint debt in new code — [x] clean
...
### C1. Naming and terminology — [x] unify
...
### C2. Where the modes live — [x] restructure the ActionBar cluster
...
### C3. Right-panel placement of BIM properties — [x] tighten
...
### C4. Level/building structure UI — [x] verify, then expose (fast-follow)
...
### C5. Scene-tree presentation — [x] label
...
### D1. Extract a shared builder-toolbar kit — [x] dedupe
...
### D2. Token and theme alignment — [x] audit
...
### D3. Accessibility pass — [x] complete
...
- [x] These changes are unrelated to builder/CAD — split into their own PR
      (see G1) with their own verification (behavior smoke in a real scene).
      Local split branch prepared: `builder-hardening/behavior-packs` at
      `a11f63b` (`Split behavior pack refactors`). Scoped behavior validation
      passed there: 38 test files, 305 passed, 1 skipped. Broader local
      validation on that branch: `NODE_OPTIONS=--max-old-space-size=8192 bun
      run typecheck` passed; `bun run lint` passed with 0 errors / 2864
      existing warnings; `BUILD_MODE=oss bunx --bun vite build` passed.
      `bun run test` still fails on the unrelated existing
      `DirectCopilotProvider.test.ts` assertion. A separate baseline fix branch
      was prepared at `builder-hardening/direct-copilot-test-fix` (`96c47c6`),
      and the stacked behavior branch
      `builder-hardening/behavior-packs-with-baseline` (`3c5055d`) now passes
      full local base validation: high-heap typecheck, `bun run lint` with
      0 errors / 2864 existing warnings, `bun run test` with 245
      files passed / 2603 tests passed / 162 skipped, and
      `BUILD_MODE=oss bunx --bun vite build`. Remote branches are published;
      PR creation is still pending because this environment lacks GitHub
      API/CLI auth.
...
`bun run lint` with 0 errors / 2827 existing warnings, `bun run test` with
231 files passed / 2572 tests passed / 162 skipped, and
`BUILD_MODE=oss bunx --bun vite build`.
Local PR 1 branch prepared: `builder-hardening/behavior-packs` at `a11f63b`;
stacked validation branch prepared and published as
`builder-hardening/behavior-packs-with-baseline` at `3c5055d`, with all base
validation commands passing locally.
Additional local path-bucket branches were prepared as staging aids:
`builder-hardening/quick-build-paths` at `bb47f48`,
`builder-hardening/mesh-cad-paths` at `50410ed`,
`builder-hardening/bim-plan-paths` at `3d04666`,
`builder-hardening/docs-site-misc-paths` at `20f6c71`,
`builder-hardening/shared-hunks` at `e85cc47`, and
`builder-hardening/owner-decision-minus-baseline` at `87fdced`. These are not
final remote PRs; they are evidence/staging branches.
Quick Build now also has a published stacked review branch:
`builder-hardening/quick-build-stacked` at `3edc789`, based on
`builder-hardening/behavior-packs-with-baseline`. That branch assigns the
Quick Build shared hunks with a Quick-only ActionBar stage and passes full local
base validation: high-heap typecheck, `bun run lint` with 0 errors / 2893
existing warnings, `bun run test` with 251 files passed / 2676 tests passed /
162 skipped, and `BUILD_MODE=oss bunx --bun vite build`.
Mesh CAD, BIM Plan, and docs/site now have published stacked review branches:
`builder-hardening/mesh-cad-stacked` at `6ec2bf3`,
`builder-hardening/bim-plan-stacked` at `c110589`, and
`builder-hardening/docs-site-misc-stacked` (docs/site branch head may move with
manifest-only amendments). All three passed local base validation.
The docs/site stack also passed `bun run test:e2e`,
`bun run test:e2e:site`, and `bun run test:e2e:builder-release`. The remaining
owner-decision bucket is explicitly out-of-scope for Builder/CAD, except
`StyledSwitch.tsx`, which moved into BIM Plan.

- [x] Split into reviewable PRs, in this order:
      1. DirectCopilot test baseline fix — independent preflight.
      2. Behavior-pack refactors + tests (F4) — independent.
      3. Quick Build (QuickBuild/* + ActionBar quick-build wiring + its smokes
         + texture script).
      4. Mesh CAD ActionBar rework (CADActionBarControls + Editor.ts CAD API
         touches).
      5. BIM Plan (PlanMode/* + RightPanel + CADToolsSection + plan smoke).
      6. Site/playground CSS + misc (Playground.tsx, globals.css, .gitignore,
         package.json scripts).
      Workstreams A–E then land as follow-up PRs against those.
- [x] Each PR runs: `bun run typecheck`, `bun run lint`, `bun run test`,
